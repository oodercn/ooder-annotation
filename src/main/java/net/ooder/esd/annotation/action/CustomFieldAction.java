...
public enum CustomFieldAction implements ActionType, CustomAction, Enumstype {
    //
    RELOAD("重新装载", "true", true),

    DYNRELOAD("重新装载", "true", true),

    POPEDITOR("弹出编辑", "true", true),

    UPLOAD("上传文件", "true", true),

    LOADCS("刷新样式", "true", true),

    LOADMENU("装载菜单", "true", false),

    ONCHANGE("动态计算", "true", false),

    ONCLICK("点击", "true", false),

    ONDATACLICK("点击数据项", "true", false),

    HOTKEY("热键", "true", false),

    LOADITEMS("装载数据项", "true", false);
    ...
    public void updateTagter(String target) {
        this.target = target;
