...
public enum GridMenu implements CustomMenu, IconEnumstype {
    ...
    DELETE("删除", "delPath", "fas fa-times", "true", IconColorEnum.PINK, new CustomAction[]{CustomGridAction.DELETE}),

    ADD("添加", "addPath", "fas fa-plus", "true", IconColorEnum.GREEN, new CustomAction[]{CustomPageAction.ADD}),

    SUBMITFORM("确定", "saveUrl", "fas fa-check", "true", IconColorEnum.CYAN, new CustomAction[]{CustomGridAction.SAVEALLROW}),

    SAVE("保存", "saveUrl", "fas fa-save", "true", IconColorEnum.BABYBLUE, new CustomAction[]{CustomFormAction.SAVE}),

    SEARCH("查询", "Search", "fas fa-search", "true", IconColorEnum.LIGHTPURPLE, new CustomAction[]{CustomGridAction.SEARCH}),

    RELOAD("刷新", "dataPath", "fas fa-sync-alt", "true", IconColorEnum.DARKBLUE, new CustomAction[]{CustomGridAction.RELOAD});
