# ooder平台PanelFormView嵌套视图结构手册

## 目录

1. [概述](#1-概述)
2. [PanelFormView嵌套机制](#2-PanelFormView嵌套机制)
3. [Field注解体系详解](#3-Field注解体系详解)
   - 3.1 GridFieldAnnotation
...
4. [嵌套视图实现方式](#4-嵌套视图实现方式)
   - 4.1 内嵌视图方式
   - 4.2 Module引用方式
5. [PanelFormView嵌套示例](#5-PanelFormView嵌套示例)
6. [最佳实践](#6-最佳实践)
7. [注意事项](#7-注意事项)
...
PanelFormView作为核心容器，支持自由布局并允许多重嵌套。PanelFormView容器嵌套时具有以下特点：
...
GridFieldAnnotation用于在PanelFormView中嵌套Grid类型的子组件。
...
```java
@GridFieldAnnotation(
    bindClass = ChildGridService.class,
...
FormFieldAnnotation用于在PanelFormView中嵌套Form类型的子组件。
...
BlockFieldAnnotation用于在PanelFormView中嵌套Block类型的子组件。
...
PanelFieldAnnotation用于在PanelFormView中嵌套Panel类型的子组件。
...
TreeFieldAnnotation用于在PanelFormView中嵌套Tree类型的子组件。
...
TabsFieldAnnotation用于在PanelFormView中嵌套Tabs类型的子组件。
...
ModuleRefFieldAnnotation用于在PanelFormView中引用其他模块的子组件。
...
ooder平台还提供了多种其他Field注解用于在PanelFormView中嵌套不同类型的组件：
...
以下是一个完整的PanelFormView嵌套示例，展示了如何在PanelFormView中嵌套多种类型的视图组件：
...
public class EmployeeInfoView {
    ...
    private WorkExperienceGrid workExperienceGrid;
    
    // 技能树（内嵌）
    @TreeFieldAnnotation(
        bindClass = SkillTreeService.class,
        borderType = BorderType.inset,
        imageClass = "fa-solid fa-tree"
    )
    @CustomAnnotation(caption = "技能树", index = 3)
    ...
    @CustomAnnotation(caption = "证书信息", index = 4)
    private CertificateTabs certificateTabs;
    
    // 绩效图表（内嵌）
    @EChartFieldAnnotation(
        chartType = EChartType.Bar
    )
    @CustomAnnotation(caption = "绩效图表", index = 5)
    ...
    @CustomAnnotation(caption = "流程图", index = 6)
    ...
    @CustomAnnotation(caption = "组织架构", index = 7)
...
通过遵循以上PanelFormView嵌套视图结构规范，可以确保ooder平台PanelFormView嵌套视图的一致性、可维护性和用户体验，为复杂业务场景提供高质量的界面解决方案。