...
列表视图用于数据展示和操作，使用@GridAnnotation注解定义。
...
```java
@GridAnnotation(
    customService = {ParentGridService.class},
    showHeader = true
)
public class ParentGrid {
    // Grid中嵌套其他视图组件
    // 通过GridFieldAnnotation等注解实现
}
```
...
#### 7.3.1 GridFieldAnnotation

GridFieldAnnotation用于在嵌套视图中定义Grid类型的子组件。
