...
Tree组合规范定义了Tree组件与其他视图组件（如Tabs、Group、Form、Grid等）协同工作的设计原则和实现方式。通过合理的组合使用，可以构建更加复杂和功能丰富的用户界面，满足多样化的业务需求。
...
public enum TreeBasedTabs implements TabItem {
    ...
    NODE_CHILDREN("子节点列表", "fas fa-list", NodeChildrenGridService.class),
...
### 5.1 设计理念
Tree与Grid的组合用于展示具有层级关系的数据列表，Tree作为分类导航，Grid展示具体的数据项。

### 5.2 实现方式
```java
@GridAnnotation(
    customService = {TreeNodeChildrenGridService.class},
    showHeader = true,
    colSortable = true,
    altRowsBg = true
)
public class TreeNodeChildrenGrid {
    @GridColumn(title = "名称", field = "name", sortable = true)
    private String name;
    
    @GridColumn(title = "类型", field = "type")
    private String type;
    
    @GridColumn(title = "创建时间", field = "createTime", formatter = "date")
    private Date createTime;
...
```java
@Service
public class TreeNodeChildrenGridService implements GridDataService<TreeNodeChildrenGrid> {
    @Autowired
    private TreeNodeSelectionService selectionService;
    
    @Override
    public List<TreeNodeChildrenGrid> loadData(GridQueryParams params) {
        TreeNode selectedNode = selectionService.getSelectedNode();
        if (selectedNode != null) {
            // 根据选中的Tree节点加载子节点数据
    ...
    private List<TreeNodeChildrenGrid> loadChildrenData(String parentId, GridQueryParams params) {
        // 实现子节点数据加载逻辑
        // 支持分页、排序、过滤等操作
    }
}
...
### 6.1 Tree-Tabs-Form-Grid组合
构建一个完整的节点管理界面，包含Tree导航、Tabs内容区域、Form编辑区和Grid数据列表。
...
public enum NodeManagementTabs implements TabItem {
    ...
    NODE_CHILDREN("子节点管理", "fas fa-list", TreeNodeChildrenGrid.class);
}
```
