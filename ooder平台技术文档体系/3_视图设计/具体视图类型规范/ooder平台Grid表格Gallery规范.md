# ooder平台Grid表格和Gallery规范

## 1. 概述

Grid表格和Gallery是ooder平台中用于数据展示的重要视图类型。Grid表格适用于结构化数据的展示和操作，而Gallery则适用于图片或卡片形式的内容展示。
...
#### @GridViewAnnotation
@GridViewAnnotation用于定义网格视图，通常用于模块入口。

#### @MGridAnnotation
@MGridAnnotation用于定义移动端的网格列表视图。

### 2.2 Grid字段注解

Grid视图中的字段通常使用标准的输入注解：
...
### 6.1 工具栏配置
使用@ToolBarMenu配置Grid/Gallery工具栏：

```java
@ToolBarMenu(
    barDock = Dock.top,
    serviceClass = GridToolBarService.class
)
```

### 6.2 底部栏配置
使用@BottomBarMenu配置Grid/Gallery底部栏：

```java
@BottomBarMenu(
    barDock = Dock.bottom,
    serviceClass = GridBarService.class
)
```
...
### 7.3 实现规范
1. **注解使用**：正确使用Grid/Gallery相关注解
2. **字段设计**：合理设计字段类型和显示方式
3. **服务分离**：视图与服务分离，职责清晰
4. **异常处理**：完善的异常处理机制

### 7.4 注意事项
1. 避免在Grid/Gallery中混合使用不兼容的注解
2. 合理配置Grid/Gallery布局属性
3. 正确使用@Uid注解标识行主键
